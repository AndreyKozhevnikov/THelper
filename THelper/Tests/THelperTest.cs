...
namespace THelper {
    ...
    public class ProjectProcessor_Tests {
        ...
        public void TryGetSolutionFiles_SLN() {
            ...
            var wrkFile = new Mock<IWorkWithFile>();
            wrkFile.Setup(x => x.EnumerateFiles(solutionPath, "*.sln", SearchOption.AllDirectories)).Returns(new string[] { @"c:\test\testSolution\testSolution\testSolution.csproj" });
        ...
        public void OpenSolution() {
            ...
            proc.OpenSolution_t();
            //assert
            wrkFile.Verify(x => x.ProcessStart(@"c:\test\testSolution\testSolution\testSolution.csproj"), Times.Once);
        }
        [Test]
        ...
        public void ProcessProject_NOtDXProj() {
            ...
            moqWrk.Setup(x => x.ProcessStart(@"c:\test\testSolution\testSolution\testSolution.csproj")).Callback(() => Assert.That(i++, Is.EqualTo(1)));
            //  moqWrk.Setup(x => x.ProcessStart(It.IsAny<ProcessStartInfo>())).Callback(() => Assert.That(i++, Is.EqualTo(3)));
            var lst2 = new List<string>();
    ...
    public class HeavyTests {
        ...
        public void Example_OpenFolder() {
            ...
            proc.ProcessArchive();
            //assert  
            Assert.AreEqual(15, callBackCounter);
        }
        ...
        public void Solution_without_dxlibs_opensolution() {
            ...
            proc.ProcessArchive();
            //assert  
            Assert.AreEqual(17, callBackCounter);
        }
        ...
        public void MainMajorMinor0_MainMajorLastVersion() {
            ...
            proc.ProcessArchive();
            //assert  
            Assert.AreEqual(21, callBackCounter);
        }
        ...
        public void MainMajorNotLastMinor_ExactConversion_NotLibraries() {
            ...
            moqFile.Setup(x => x.ProcessStart(@"c:\test\testSolution\testSolution\testSolution.csproj")).Do((x3) => { callOrderDictionary["ProcessStart1"] = orderCounter++; }).Callback(() => Assert.That(callBackCounter++, Is.EqualTo(callOrderDictionary["ProcessStart1"])));
            //act  
            proc.ProcessArchive();
            //assert  
        ...
        public void NotIstalledMajorZeroMinor_OpenFolder_WrongKey() {
            ...
            moqMessage.Setup(x => x.ConsoleReadKey(false)).Returns(ConsoleKey.D8).Do((x3) => { callOrderDictionary["ConsoleReadKey"] = orderCounter++; }).Callback(() => Assert.That(callBackCounter++, Is.EqualTo(callOrderDictionary["ConsoleReadKey"])));
            moqCSProj.Setup(x => x.DisableUseVSHostingProcess()).Do((x3) => { callOrderDictionary["DisableUseVSHostingProcess"] = orderCounter++; }).Callback(() => Assert.That(callBackCounter++, Is.EqualTo(callOrderDictionary["DisableUseVSHostingProcess"])));
            moqCSProj.Setup(x => x.RemoveLicense()).Do((x3) => { callOrderDictionary["RemoveLicense"] = orderCounter++; }).Callback(() => Assert.That(callBackCounter++, Is.EqualTo(callOrderDictionary["RemoveLicense"])));
